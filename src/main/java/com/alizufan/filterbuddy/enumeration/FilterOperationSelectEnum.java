...
@Getter
public enum FilterOperationSelectEnum {
    Is("is", "is"),
    IsNot("isNot", "is not"),
    ...
    FilterOperationSelectEnum(String id, String label) {
        this.id = id;
        this.label = label;
    }

    public static FilterOperation findOperation(String id) {
        FilterOperationSelectEnum d = FilterOperationSelectEnum.find(id);
        if (d == null) return null;
        return new FilterOperation(d.id, d.label);
    }

    public static FilterOperationSelectEnum find(String id) {
        Optional<FilterOperationSelectEnum> value = Arrays
            .stream(values())
            .filter(v -> {
