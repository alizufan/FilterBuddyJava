...
@Getter
public enum FilterOperationDateEnum {
    Is("is", "is"),
    ...
    FilterOperationDateEnum(String id, String label) {
        this.id = id;
        this.label = label;
    }

    public static FilterOperation findOperation(String id) {
        FilterOperationDateEnum d = FilterOperationDateEnum.find(id);
        if (d == null) return null;
        return new FilterOperation(d.id, d.label);
    }

    public static FilterOperationDateEnum find(String id) {
        Optional<FilterOperationDateEnum> value = Arrays
            .stream(values())
            .filter(v -> {
